import { BaseComponent, BodyText1, LightText, Title1, Title3 } from "@/components/base/baseComponent";
...
import { ClassifyLabelEnum, getCreatures, getCreaturesById } from "@/api/creatures";
  ...
  return (
    <CreatureDetailContainer>
      ...
      <BaseComponent>
        ...
        <ClassificationItem title="Loai" value={creature?.speciesBelong?.name_vn} />
        <Divider sx={{ borderStyle: "solid", borderBottomWidth: "1px" }} />

        <ClassificationItem title="Lop" value={creature?.groupBelong?.name_vn} />
        <Divider sx={{ borderStyle: "solid", borderBottomWidth: "1px" }} />

        <ClassificationItem title="Bo" value={creature?.setBelong?.name_vn} />
        <Divider sx={{ borderStyle: "solid", borderBottomWidth: "1px" }} />

        <ClassificationItem title="Ho" value={creature?.familyBelong?.name_vn} />
      ...
      <BaseComponent>
        ...
        {(creature?.description || "").trim() ? <div dangerouslySetInnerHTML={{__html: creature.description?.replaceAll("<br />", "") }}>
        </div> : <LightText>* Hien dang cap nhap</LightText>}
      </BaseComponent>
      ...
      <BaseComponent>
        ...
        <div>
          <ListCreatures creatures={listCreature} isDetailPage loading={false} />
        </div>
      </BaseComponent>
      <Footer />
    </CreatureDetailContainer>
...
export const getServerSideProps = wrapper.getServerSideProps((store) => async (context: GetServerSidePropsContext) => {
  ...
  try {
    if(id && typeof id === 'string' && !Number.isNaN(parseInt(id, 10))) {
      ...
      listCreature = res?.data?.filter((item: any) => item.id !== creature.id)
    }
  } catch(err) {
